...
#each_alphabet_repeated = 260
#total_iterations = (numWords - number_of_alphabets) + (number_of_alphabets * each_alphabet_repeated)
#alphabet_iter = 1
word_iter = 0

for i in range(9990000000,numWords+9990000000):
# for i in range(0, total_iterations):
    if word_iter%1000==0:
        print 'completed ', word_iter
        thousand=word_iter/1000
    ...
    #new code for vertical
    textWord = words[word_iter]
    word_iter += 1
    word_to_print = textWord
    l = len(textWord)
    t = []
    additional_char = [' ̑','്','ാ','ി','ീ','ു','ൂ','ൃ','ൄ','ൢ','ൣ','െ','േ','ൈ','ൊ','ോ','ൗ','ം','ഃ','്യ','്ര','്വ','ി']
    main_char = ['അ','ക','ഖ','ഗ','ഘ','ങ','ച','ഛ','ജ','ഝ','ഞ','ട','ഠ','ഡ','ഢ','ണ','ത','ഥ','ദ','ധ','ന','പ','ഫ','ബ','ഭ','മ','യ','ര','ല','വ','ശ','ഷ','സ','ഹ','ള','ഴ','റ','ഇ','ഈ','ഉ','ഊ','ശൂ','ഹൂ','എ','ഏ','ഐ','ഒ','ഓ','ഔ','അം','അഃ']
    for w in range(0,l):
        if w<l-1: 
            if textWord[w+1].encode('utf-8') in additional_char:
                # print('true')
                t.append(textWord[w])
            else:
                # print('char:',additional_char[1][0])
                # print(textWord[w])
                # if textWord[w].encode('utf-8') == main_char[1]:
                #     print('Ka matched')
                t.append(textWord[w] + '\n')
        else:
            t.append(textWord[w])
