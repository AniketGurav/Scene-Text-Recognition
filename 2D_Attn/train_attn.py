...
def val(net, test_dataset, criterion, max_iter=100):
    ...
    for i in range(max_iter):
        ...
        #preds = crnn(image, t[:, :-1],is_train=False)  # align with Attention.forward
        preds = crnn(image, text_for_pred, is_train=False)
...
#    cost = criterion(preds, text.to(device), preds_size, length) / batch_size
    preds = crnn(image, t[:, :-1])  # align with Attention.forward
    target = t[:, 1:]  # without [GO] Symbol
